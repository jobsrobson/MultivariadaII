...
# Título do Dashboard
# ---------------------------
st.title("Frota e Acidentes por Município")
st.markdown(
    "Visualização de frota total e acidentes (totais e com óbito) por município.")
...
# STREAMLIT - TÍTULO
# ===========================================
st.title("Análise Fatorial - Dashboard Interativo")
...
# ===========================================
st.subheader("Heatmap das Cargas Fatoriais")
fig, ax = plt.subplots(figsize=(10, 8))
sns.heatmap(cargas, annot=True, cmap='RdBu', center=0,
...
# ===========================================
st.subheader("Variância Explicada pelos Fatores")
...
fig_var = px.bar(
    ...
    title="Variância Acumulada por Fator",
    color='Fator'
)
fig_var.update_traces(texttemplate='%{text:.2f}', textposition='outside')
fig_var.update_layout(yaxis=dict(range=[0, 1]))
...
# TABELA COM CARGAS
# ===========================================
st.subheader("Tabela de Cargas Fatoriais")
...
st.markdown("""No output do CCA, obtem-se pares canônicos, que medem scores independentes,
        logo segue-se a análise com foco no F1, de maneira mais objetiva.""")
# Correlação canônica
st.subheader('Correlação Canônica')
st.write('Brasília:', resultados_cca['Brasília']['cor'])
st.write('Outras Cidades:', resultados_cca['Outras']['cor'])

# Loadings X
st.subheader('Loadings X - Brasília')
st.dataframe(resultados_cca['Brasília']['load_X'])
st.subheader('Loadings X - Outras Cidades')
st.dataframe(resultados_cca['Outras']['load_X'])
st.markdown("""Os loadings reconheceram como as 5 variáveis que melhor explicam o próprio grupo de explicativas, em ordem: automovel, caminhao, caminhao trator, caminhonete e camioneta.
...
# Loadings Y
st.subheader('Loadings Y - Brasília')
st.dataframe(resultados_cca['Brasília']['load_Y'])
st.subheader('Loadings Y - Outras Cidades')
st.dataframe(resultados_cca['Outras']['load_Y'])
st.markdown("""Diante o grupo das variáveis Y, em ambas a melhor explicativa(próximo a |1|) é a variável quantidade de acidentes, onde os acidentes com óbitos ficam em segundo.
        Porém, nas outras cidades os óbitos possuem um peso maior quando comparados à Brasília em suas cargas canônicas.""")


# Cross-loadings X->Y
st.subheader('Cross-loadings X -> Y - Brasília')
st.dataframe(resultados_cca['Brasília']['cross_XY'])
st.subheader('Cross-loadings X -> Y - Outras Cidades')
st.dataframe(resultados_cca['Outras']['cross_XY'])

st.markdown("""No caso de Brasília, as variáveis que realmente se destacam na hora de prever a quantidade de acidentes são, em ordem de importância: micro-ônibus, triciclo, automóvel, reboque, semi-reboque, ônibus, caminhão-trator, caminhonete, caminhão, camioneta, utilitário, motocicleta e motoneta. Essas categorias foram as que apresentaram os maiores escores e, por isso, têm maior peso na explicação da variação de acidentes na cidade.
...
# Cross-loadings Y->X
st.subheader('Cross-loadings Y -> X - Brasília')
st.dataframe(resultados_cca['Brasília']['cross_YX'])
st.subheader('Cross-loadings Y -> X - Outras Cidades')
st.dataframe(resultados_cca['Outras']['cross_YX'])

st.markdown("""Um ponto interessante aparece ao analisar os cross-loadings do Fator 2 (F2). Nele, a variável acidentes com óbitos apresenta um score bem alto e negativo (cerca de –0,9), enquanto o total de acidentes praticamente não se relaciona com esse fator (valor próximo de zero). Isso mostra que o F2 está muito mais associado à gravidade dos acidentes do que à quantidade total de ocorrências.

Quando observamos as variáveis preditoras ligadas ao F2 em Brasília, quase nenhuma apresenta força suficiente para ser considerada relevante na predição. Ainda assim, existe uma ligação leve — mais descritiva do que explicativa — com alguns tipos de veículos: quadriciclo (0,63), trator de rodas (0,54) e ciclomotor (0,52). Esses valores indicam apenas uma tendência de associação, mas não configuram variáveis fortes para prever acidentes com óbitos dentro desse fator.""")


# Gráfico do Par Canônico 1
st.subheader('Gráfico Par Canônico 1')
fig_BR = px.scatter(
...
st.markdown("""O scatter reflete em Brasília uma relação perfeita, pontos quase colineares, onde a variabilidade é baixa ou estrutura muito alinhada entre X e Y.

Já para outras cidades, percebe-se uma relação forte, mas mais natural, com dispersão, logo maior heterogeneidade nas relações entre frota e acidentes.""")